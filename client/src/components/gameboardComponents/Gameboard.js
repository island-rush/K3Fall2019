...
class Gameboard extends Component {
	...
	imageSize = { x: 3.4, y: 2.75 };
	hexagonSize = { x: 3.15, y: 3.15 };
	...
	render() {
		const positions = Object.keys(this.props.gameboard).map(positionIndex => (
			<Hexagon
				key={positionIndex}
				posId={0}
				q={this.qIndexSolver(positionIndex)}
				r={this.rIndexSolver(positionIndex)}
				s={-999}
				fill={this.patternSolver(this.props.gameboard[positionIndex])}
				onClick={() => {
					this.props.selectPosition(positionIndex);
				}}
				className={
					this.props.gameboardMeta.selectedPosition == parseInt(positionIndex)
						? "selectedPos"
						: ""
				}
			/>
